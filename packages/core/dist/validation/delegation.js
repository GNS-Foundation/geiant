...
export function createSubDelegation(parentCert, subAgentPublicKey, scopeReduction) {
    ...
    const effectiveUntil = scopeReduction?.validUntil
        ? new Date(Math.min(new Date(parentCert.validUntil).getTime(), new Date(scopeReduction.validUntil).getTime())).toISOString()
...
// Helpers
// ---------------------------------------------------------------------------
function checkCertFormat(cert) {
    if (!cert.id)
        return { valid: false, reason: 'Missing cert ID' };
