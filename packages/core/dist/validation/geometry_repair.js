 ...
 */
function repairTransposedCoordinates(geom) {
    const before = JSON.parse(JSON.stringify(geom.coordinates));
    const swapCoord = (c) => [c[1], c[0]];
    const swapAll = (coords) => coords.map(swapCoord);
 ...
 */
function repairUnclosedRing(geom) {
 ...
 */
function repairSelfIntersection(geom) {
    if (geom.type !== 'Polygon' && geom.type !== 'MultiPolygon') {
 ...
 */
function repairDuplicatePoints(geom) {
    const before = JSON.parse(JSON.stringify(geom.coordinates));
    const dedup = (coords) => {
        if (coords.length === 0)
            return coords;
    ...
    switch (geom.type) {
        ...
        case 'MultiPolygon': {
            const g = geom;
            const polys = g.coordinates.map(poly => poly.map(ring => {
                const d = dedup(ring);
                coordinatesChanged += ring.length - d.length;
                return d;
            }));
 ...
 */
function repairInvalidCoordinate(geom) {
    const before = JSON.parse(JSON.stringify(geom.coordinates));
    const clamp = (c) => [
        Math.max(-180, Math.min(180, c[0])),
        Math.max(-90, Math.min(90, c[1])),
    ];
    const clampAll = (coords) => coords.map(clamp);
    let repairedGeometry;
    let coordinatesChanged = 0;
    const countChanged = (original, fixed) => original.filter((c, i) => c[0] !== fixed[i][0] || c[1] !== fixed[i][1]).length;
    switch (geom.type) {
        case 'Point': {
            const g = geom;
...
// Helper
// ---------------------------------------------------------------------------
function noRepair(geom, errorType, reason) {
    return {
        repaired: false,
...
//# sourceMappingURL=geometry_repair.js.map