...
class SpatialMemoryGraph {
    ...
    async create(params) {
        ...
        const breadcrumb = buildBreadcrumb(node, params.taskId, 'geometry_mutated', params.delegationCertHash);
    ...
    async mutate(params) {
        ...
        const eventType = crossedBoundary
            ...
            : 'geometry_mutated';
        const breadcrumb = buildBreadcrumb(node, params.taskId, eventType, params.delegationCertHash);
    ...
    async getHistory(featureId) {
        ...
        if (!headHash)
            return [];
        const history = [];
        let current = this.nodes.get(headHash);
        while (current) {
    ...
    async getAtTime(featureId, isoTimestamp) {
 ...
 */
function extractH3Cells(feature, resolution) {
    const cells = new Set();
 ...
 */
function detectBoundaryCrossing(oldCells, newCells) {
 ...
 */
function agentSign(agentPublicKey, hash) {
    if (!hash)
        return '0'.repeat(128);
...
// Breadcrumb factory
// ---------------------------------------------------------------------------
function buildBreadcrumb(node, taskId, eventType, delegationCertHash) {
