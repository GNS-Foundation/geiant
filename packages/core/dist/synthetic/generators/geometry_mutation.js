...
// Step builders
// ---------------------------------------------------------------------------
function validStep(step, op, geometry, desc) {
    return { step, operation: op, geometry, description: desc, is_valid: true };
}
function invalidStep(step, op, geometry, desc, errorType) {
...
export function generateGeometryMutationScenarios() {
    ...
    ], undefined, 'route_success', 'All three steps contain valid closed polygons. Chain should reach length 3 with no rejection.', 'easy', ['clean-workflow', 'buffer', 'rome', 'no-error']));
    records.push(geomScenario('Clean 5-step Terna grid zone analysis workflow', 'terna-clean-5step', [
    ...
    ], undefined, 'route_success', 'Five clean steps. All geometries valid. Chain length should be 5.', 'easy', ['clean-workflow', 'terna', 'grid', '5-step', 'no-error']));
    // ── Error at step 0 (genesis invalid) ─────────────────────────────────────
    records.push(geomScenario('Self-intersecting bowtie at genesis — rejected immediately', 'bowtie-at-genesis', [
    ...
    ], 0, 'reject_geometry', 'Genesis geometry is a bowtie — edges cross. GEIANT GeometryGuard rejects at step 0 before any node is created.', 'easy', ['self-intersection', 'bowtie', 'genesis-invalid', 'geometry-guard']));
    records.push(geomScenario('Unclosed ring at genesis — missing closing coordinate', 'unclosed-at-genesis', [
        invalidStep(0, 'create', UNCLOSED_RING, 'Create zone (INVALID: ring not closed)', 'unclosed_ring'),
    ], 0, 'reject_geometry', 'Polygon ring has 4 coordinates but last != first. Classic LLM error — model generates [A,B,C,D] instead of [A,B,C,D,A]. Rejected at genesis.', 'easy', ['unclosed-ring', 'llm-error', 'genesis-invalid', 'geometry-guard']));
    // ── Error injected mid-workflow ────────────────────────────────────────────
    records.push(geomScenario('Valid start, self-intersection injected at step 2 of 4', 'bowtie-midflow', [
    ...
    ], 2, 'reject_geometry', 'Steps 0-1 succeed and create 2 memory nodes. Step 2 produces invalid geometry — GEIANT rejects mutation, preserving the valid chain at length 2. The corruption is caught before it enters the graph.', 'medium', ['self-intersection', 'mid-workflow', 'chain-preserved', 'geometry-guard', 'llm-corruption']));
    records.push(geomScenario('Valid start, unclosed ring injected at step 3 of 5', 'unclosed-midflow', [
    ...
    ], 3, 'reject_geometry', 'Three valid nodes created before error. Step 3 mutation rejected. Graph preserves 3-node chain. Agent receives structured error with suggestion to close the ring.', 'medium', ['unclosed-ring', 'mid-workflow', '3-node-preserved', 'terna', 'geometry-guard']));
    // ── Coordinate hallucination ───────────────────────────────────────────────
    records.push(geomScenario('LLM hallucinated coordinate (lng=200) at step 1', 'hallucinated-coord', [
        validStep(0, 'create', ROME_CENTRO, 'Create Rome zone'),
        invalidStep(1, 'buffer', HALLUCINATED_COORDS, 'Buffer result (INVALID: LLM hallucinated lng=200)', 'invalid_coordinate'),
    ], 1, 'reject_geometry', 'Step 1 geometry has longitude=200 which is outside WGS84 bounds (-180 to 180). Classic LLM coordinate hallucination — model invents coordinates that look plausible but are geographically impossible.', 'easy', ['hallucination', 'invalid-coordinate', 'wgs84', 'llm-error', 'geometry-guard']));
    records.push(geomScenario('Coordinate transposition at step 2 — lat/lng swapped', 'transposed-coords', [
        validStep(0, 'create', ROME_CENTRO, 'Create Rome zone'),
    ...
    ], 2, 'reject_geometry', 'Agent reprojects polygon but swaps lat/lng order. Coords like [41.89, 12.46] instead of [12.46, 41.89]. GEIANT heuristic detects the swap and returns a suggestion with the corrected coordinate order.', 'medium', ['transposition', 'lat-lng-swap', 'reprojection', 'llm-error', 'geometry-guard']));
    // ── Territory boundary crossing ────────────────────────────────────────────
    records.push(geomScenario('Rome zone drifts to Milan over 3 steps — boundary crossing detected', 'boundary-drift', [
        validStep(0, 'create', ROME_CENTRO, 'Create Rome zone'),
    ...
    'flag_boundary_crossing', 'All three geometries are individually valid. But the centroid moves from Rome (41.9°N, 12.5°E) to Milan (45.5°N, 9.2°E) — ~480km. H3 cell overlap drops to <5%, triggering territory_boundary_crossed breadcrumb at step 2.', 'medium', ['boundary-crossing', 'drift', 'rome-to-milan', 'breadcrumb', 'valid-geometry']));
    // ── Adversarial ────────────────────────────────────────────────────────────
    records.push(geomScenario('ADVERSARIAL: Near-degenerate polygon — valid but suspicious', 'near-degenerate', [
        validStep(0, 'create', ROME_CENTRO, 'Create Rome zone'),
        validStep(1, 'simplify', NEAR_DEGENERATE, 'Simplify result — near-degenerate but technically valid'),
    ], undefined, 'route_success', 'Near-degenerate polygon (width ~0.0000001 degrees = ~1cm) is technically valid GeoJSON — passes all validation checks. This is a known limitation of pure geometry validation without semantic context. Future: area threshold check.', 'adversarial', ['near-degenerate', 'adversarial', 'valid-but-suspicious', 'future-improvement']));
    records.push(geomScenario('ADVERSARIAL: 10-step workflow, error at random middle step', 'ten-step-random-error', [
    ...
    ], 5, 'reject_geometry', '10-step workflow. Error injected at step 5. GEIANT should build a 5-node chain (steps 0-4), then reject step 5. The adversarial test: without GEIANT, the corrupted geometry silently enters the workflow and corrupts all downstream steps.', 'adversarial', ['10-step', 'mid-workflow', 'adversarial', 'self-intersection', 'chain-integrity', 'llm-corruption']));
    return records;
}
//# sourceMappingURL=geometry_mutation.js.map