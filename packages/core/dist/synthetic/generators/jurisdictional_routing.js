...
function generateJurisdictionalRoutingScenarios() {
    ...
    }, 'easy', ['italy', 'gdpr', 'eu-ai-act', 'grid', 'direct-match']));
    records.push(scenario('Finance task in Zurich — FINMA jurisdiction, certified required', 'zurich', 'finance', 'certified', [AGENT_POOL[2], AGENT_POOL[3]], // milan finance (trusted) + swiss finance (certified)
    'route_success', {
    ...
    }, 'medium', ['switzerland', 'finma', 'finance', 'tier-enforcement', 'swiss-dpa']));
    records.push(scenario('Health task in Berlin — GDPR + EU AI Act, certified agent available', 'berlin', 'health', 'trusted', [AGENT_POOL[4]], // health@eu-north
    'route_success', {
    ...
    }, 'easy', ['germany', 'gdpr', 'health', 'eu-ai-act']));
    records.push(scenario('Compliance audit in Berlin — sovereign agent, GDPR jurisdiction', 'berlin', 'compliance', 'certified', [AGENT_POOL[5]], // compliance@berlin (sovereign)
    'route_success', {
    ...
    }, 'hard', ['germany', 'gdpr', 'compliance', 'sovereign-in-restricted-jurisdiction']));
    // ── Category 2: Cross-border ambiguity ───────────────────────────────────
    records.push(scenario('Finance task near CH/IT border — Lugano area', 'geneva', 'finance', 'trusted', [AGENT_POOL[2], AGENT_POOL[3]], // milan finance + swiss finance
    'route_success', {
        expected_ant_handle: 'finance@swiss',
        expected_country: 'CH',
        expected_frameworks: ['SWISS_DPA', 'FINMA'],
        explanation: 'Geneva resolves to CH (Switzerland), not IT. FINMA governs. finance@swiss covers the territory.',
    }, 'medium', ['switzerland', 'cross-border', 'finma', 'border-ambiguity']));
    records.push(scenario('Legal task in London — UK GDPR post-Brexit, no EU AI Act', 'london', 'legal', 'trusted', [AGENT_POOL[7]], // legal@london
    'route_success', {
    ...
    }, 'medium', ['uk', 'uk-gdpr', 'post-brexit', 'legal', 'not-eu-ai-act']));
    // ── Category 3: No eligible agent ────────────────────────────────────────
    records.push(scenario('Grid task in Naples — only provisioned agent available', 'naples', 'grid', 'trusted', [AGENT_POOL[8]], // grid@naples (provisioned)
    'reject_tier', {
    ...
    }, 'easy', ['italy', 'tier-insufficient', 'provisioned', 'reject']));
    records.push(scenario('Environment task in Singapore — no environment agent covers SG', 'singapore', 'environment', 'observed', [AGENT_POOL[6]], // environment@eu (covers Paris area only)
    'reject_no_ant', {
        expected_ant_handle: undefined,
        expected_country: 'SG',
    ...
    }, 'medium', ['singapore', 'pdpa', 'no-coverage', 'reject', 'apac']));
    records.push(scenario('Finance task in New York — no finance agent covers US territory', 'new_york', 'finance', 'observed', [AGENT_POOL[2], AGENT_POOL[3]], // milan + swiss finance
    'reject_no_ant', {
        expected_ant_handle: undefined,
        expected_country: 'US',
    ...
    }, 'easy', ['usa', 'no-coverage', 'reject', 'us-eo-14110']));
    // ── Category 4: Unknown jurisdiction ─────────────────────────────────────
    records.push(scenario('Task in Atlantic Ocean — no jurisdiction resolvable', 'atlantic', 'general', 'observed', AGENT_POOL, 'reject_no_jurisdiction', {
        expected_country: 'XX',
        expected_frameworks: [],
        explanation: 'Atlantic Ocean H3 cell has no country mapping. Router cannot resolve jurisdiction. Task rejected at Gate 2.',
    }, 'easy', ['ocean', 'no-jurisdiction', 'reject', 'gate-2']));
    // ── Category 5: Multi-framework compliance conflict ───────────────────────
    records.push(scenario('Sovereign grid agent in Rome — GDPR caps at trusted', 'rome', 'grid', 'sovereign', [
    ...
    }, 'hard', ['gdpr-compliance-conflict', 'sovereign', 'italy', 'framework-enforcement', 'adversarial']));
    records.push(scenario('Health task in São Paulo — LGPD jurisdiction, no local agent', 'sao_paulo', 'health', 'observed', [AGENT_POOL[4]], // health@eu-north (Germany)
    'reject_no_ant', {
        expected_ant_handle: undefined,
        expected_country: 'BR',
    ...
    }, 'medium', ['brazil', 'lgpd', 'latam', 'no-coverage', 'reject']));
    ...
    }, 'adversarial', ['territory-mismatch', 'adversarial', 'cross-jurisdiction', 'italy', 'switzerland']));
    records.push(scenario('ADVERSARIAL: Agent tier self-reported as certified but ops count = 10', 'milan', 'finance', 'certified', [
    ...
    }, 'adversarial', ['tier-spoofing', 'adversarial', 'trust-boundary', 'registry-validation']));
    return records;
}
//# sourceMappingURL=jurisdictional_routing.js.map