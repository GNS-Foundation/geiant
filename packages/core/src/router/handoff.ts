...
import { gridDisk, cellToLatLng, gridDistance } from 'h3-js';
import {
  GeiantTask,
  HandoffDecision,
  HandoffCert,
...
const ROUTER_PRIVATE_KEY_HEX =
  process.env.ROUTER_SIGNING_KEY ??
...
export async function resolveHandoff(
  task: GeiantTask,
  originJurisdiction: JurisdictionResult,
  ...
  const MAX_RINGS = 3;
  const searchedCells = new Set<string>([task.originCell]);
  ...
  for (let ring = 1; ring <= MAX_RINGS; ring++) {
    ...
    for (const cell of ringCells) {
      ...
      if (!candidateJurisdiction) continue;

      // Skip cells in same jurisdiction as origin — we already know no ant is there
      if (candidateJurisdiction.countryCode === originJurisdiction.countryCode) continue;
...
function issueHandoffCert(
  task: GeiantTask,
  receivingAnt: AntManifest,
  targetCell: H3Cell,
  targetJurisdiction: JurisdictionResult,
 ...
 */
function isHandoffCompatible(
  from: JurisdictionResult,
  to: JurisdictionResult,
  task: GeiantTask
): CompatibilityResult {
  ...
  if (fromFrameworkIds.includes('GDPR')) {
    const hasEquivalent = toFrameworkIds.some(id =>
      ['GDPR', 'UK_GDPR', 'SWISS_DPA', 'PDPA_SG'].includes(id)
    );
    if (!hasEquivalent) {
      return {
        ok: false,
  ...
  if (task.requiredFacet === 'health') {
    const hasHealth = toFrameworkIds.some(id =>
      ['GDPR', 'UK_GDPR', 'SWISS_DPA', 'CCPA'].includes(id)
    );
    if (!hasHealth) {
      return {
        ok: false,
...
function selectBestAnt(ants: AntManifest[], cell: H3Cell): AntManifest {
  // Score by: compliance score + operation count (proxy for experience)
  return ants.sort((a, b) =>
    (b.complianceScore + Math.log10(b.operationCount + 1)) -
    (a.complianceScore + Math.log10(a.operationCount + 1))
  )[0];
}
 ...
 */
function findBridgeCells(originCell: H3Cell, targetCell: H3Cell, ringDistance: number): H3Cell[] {
  try {
...
export function formatHandoffSummary(handoff: HandoffDecision): string {
  if (!handoff.possible) {
