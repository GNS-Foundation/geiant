...
function makeTestAnt(publicKey: string, handle: string): AntManifest {
  return {
    identity: {
      publicKey,
      handle,
...
describe.skipIf(skip)('SupabaseRegistry — integration', () => {
  ...
  it('hasAntsForFacet returns false for unknown facet', async () => {
    const has = await registry.hasAntsForFacet('mobility' as any);
  ...
  it('territory cells are persisted and retrieved correctly', async () => {
    ...
    const retrieved = await registry.get(TEST_KEY);
    expect(retrieved!.identity.territoryCells).toEqual(
      expect.arrayContaining(['851e8053fffffff', '851e8050ca7ffff'])
    );
  });
});
...
describe('SupabaseRegistry — unit (no network)', () => {
  ...
  it('rejects invalid manifest on register', async () => {
    ...
    const bad = { identity: { publicKey: 'tooshort', handle: 'x', facet: 'grid', tier: 'trusted', territoryCells: [], provisionedAt: '' }, description: '', capabilities: [], mcpEndpoints: [], operationCount: 0, complianceScore: 0, signature: '', updatedAt: '' } as any;
