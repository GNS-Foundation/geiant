...
function makePolygon(coords: [number, number][][]): SpatialGeometry {
  return { type: 'Polygon', coordinates: coords };
}

function makeFeature(geom: SpatialGeometry): SpatialFeature {
  return { type: 'Feature', geometry: geom, properties: {} };
}

// Rome bounding box — valid closed polygon
const ROME_POLYGON: SpatialGeometry = {
  type: 'Polygon',
  coordinates: [[
...
const BOWTIE_POLYGON: SpatialGeometry = {
  type: 'Polygon',
  coordinates: [[
...
const UNCLOSED_POLYGON: SpatialGeometry = {
  type: 'Polygon',
  coordinates: [[
...
const TRANSPOSED_POINT: SpatialGeometry = {
  type: 'Point',
  coordinates: [41.9, 12.5], // lat, lng — WRONG
};

// Duplicate consecutive points
const DUPLICATE_LINESTRING: SpatialGeometry = {
  type: 'LineString',
  coordinates: [
...
function makeRomeCert(overrides: Partial<DelegationCert> = {}): DelegationCert {
  return {
    id: 'cert-001',
...
function makeTask(cell: string, facet = 'grid', overrides: Partial<GeiantTask> = {}): GeiantTask {
  return {
    id: crypto.randomUUID(),
    originCell: cell,
    requiredFacet: facet as any,
    minTier: 'observed',
...
describe('L2 Self-Healing — repairGeometry', () => {
  ...
  describe('coordinate transposition repair', () => {
    it('repairs transposed Point coordinates', () => {
      ...
      const fixed = result.repairedGeometry as { type: 'Point'; coordinates: [number, number] };
    ...
    it('repairs transposed Polygon', () => {
      const transposedPolygon: SpatialGeometry = {
        type: 'Polygon',
        coordinates: [[
      ...
      expect(result.repaired).toBe(true);
      const fixed = result.repairedGeometry as { type: 'Polygon'; coordinates: [number, number][][] };
  ...
  describe('duplicate points repair', () => {
    it('removes consecutive duplicate points from LineString', () => {
      ...
      const fixed = result.repairedGeometry as { type: 'LineString'; coordinates: [number, number][] };
...
describe('L2 Self-Healing — repairFeatures (batch)', () => {
  ...
  it('marks allRepaired=false when repair fails', () => {
    ...
    const features = [makeFeature({ type: 'Polygon', coordinates: [] } as any)];
...
describe('L1 Cross-Jurisdictional Hand-off — resolveHandoff', () => {
  ...
  it('finds a handoff from an unserved cell to an adjacent served jurisdiction', async () => {
    ...
    if (!originJurisdiction) return; // skip if cell not resolvable
...
describe('Router integration — L2 repair + L1 handoff', () => {
  ...
  it('routes task with unclosed polygon after auto-repair', async () => {
    ...
    const decision = await router.route(task) as HandoffRoutingDecision;
  ...
  it('rejects task with unrepairable geometry', async () => {
    const romeCell = '851e8053fffffff';
    const emptyGeom: SpatialGeometry = { type: 'Polygon', coordinates: [] };
    const task = makeTask(romeCell, 'grid', {
      delegationCert: makeRomeCert({ scopeCells: [romeCell] }),
