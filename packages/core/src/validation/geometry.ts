...
import { SpatialFeature, SpatialGeometry, ValidationResult, GeometryErrorType } from '../types/index.js';
 ...
 */
export function validateGeometries(features: SpatialFeature[]): ValidationResult {
...
export function validateFeature(feature: SpatialFeature, index = 0): ValidationResult {
  if (!feature.geometry) {
    return {
      valid: false,
      featureIndex: index,
...
export function validateGeometry(geom: SpatialGeometry, featureIndex = 0): ValidationResult {
  switch (geom.type) {
    case 'Point':
...
function validatePoint(coord: [number, number], featureIndex: number): ValidationResult {
  const [x, y] = coord;
...
function hasDuplicateConsecutivePoints(coords: [number, number][]): boolean {
 ...
 */
function detectSelfIntersection(ring: [number, number][]): boolean {
...
export function looksTransposed(lng: number, lat: number): boolean {
...
export function formatValidationError(result: ValidationResult): string {
  if (result.valid) return 'Geometry is valid.';
  return [
