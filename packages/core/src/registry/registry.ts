...
// ---------------------------------------------------------------------------
// Registry interface — swap implementation without touching the router
// ---------------------------------------------------------------------------
...
export class InMemoryRegistry implements AgentRegistry {
  private ants = new Map<string, AntManifest>();
  ...
  async hasAntsForFacet(facet: AntFacet): Promise<boolean> {
    return Array.from(this.ants.values()).some(
      ant => ant.identity.facet === facet || ant.identity.facet === 'general'
    );
  }
...
export function setRegistry(registry: AgentRegistry): void {
  _registry = registry;
}
...
export async function seedDevRegistry(registry: AgentRegistry): Promise<void> {
